...
> ⚠️ **指标口径限制（非无泄漏）**：V5 使用了压测满意度特征
> （`packet_loss_satisfaction_bucket` ← `overall_packet_loss_benchmark_satisfaction_pct`），
> 该特征沿用 RJ 口径，取"每节点最新一次 netbench 结果"并复用到整个训练窗口，**未按日做 as-of 对齐**
> （`节点业务推荐模型V5全流程说明.md` §5.7 已明示此限制）。
> 因此上表 RMSE/R² 含**未来信息风险**，只能作为"同口径相对比较"，**不得解释为无泄漏的线上效果**。
> 消除该风险需按日重建压测快照（新工作，未做）。

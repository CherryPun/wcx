...
- 历史覆盖：约 5% 节点抽样（后缀 00–0c），满 7 天账；起点均为 2026-03-16。

## 数据拉取截止（各源截至日不同）

数据为按需现拉的快照，不是自动滚动更新；重跑/复现前请按本表核对是否需重新拉取。

| 数据文件 / 来源 | 最近拉取截止日 | 说明 |
|---|---|---|
| `outcomes_7d_named.csv`（业务起账日） | 2026-08-28 | 训练/主版标签；节点最早上线日截止 2026-08-26 |
| `bw_daily_7d.csv` | 2026-09-02 | 近 7 天日 95 带宽 |
| `current_ops_30d_5171.csv`（宽表导出） | 2026-09-06 | 结算宽表 `node_day_ops_wide_full` 当日最新入库日 |
| `cur7d_map.csv`（“当前在跑”列映射） | 2026-09-06 | 判定窗口截至宽表最新日 |
| 控制面快照 `jarvis.node_analysis_data` | 2026-09-07 | 当日数据可能不完整（进行中） |

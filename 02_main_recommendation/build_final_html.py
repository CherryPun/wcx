...
<div class="card">共 {len(d)} 个节点。Top1~3 来自<b>相似节点真实跑过的业务</b>候选，
按<b>矿主预测 7 天结算</b>主序 + 平台利润/容量软降权。数值为预测（7天金额）。定位：候选/参考，不承诺最赚。</div>
<p><input id="q" placeholder="过滤 node_id 或业务" oninput="f()"></p>
<table><thead><tr><th>node_id</th><th>Top1</th><th>Top2</th><th>Top3</th></tr></thead>
<tbody>{''.join(tbody)}</tbody></table>
<script>function f(){{var q=document.getElementById('q').value.toLowerCase();document.querySelectorAll('tbody tr').forEach(tr=>tr.style.display=tr.innerText.toLowerCase().indexOf(q)>=0?'':'none')}}</script>
</body></html>"""

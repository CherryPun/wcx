...
> 最新闭环已刷新到 `2026-09-08`。31 天主模型使用
> `2026-08-09 ~ 2026-09-08`，并与 `2026-08-26 ~ 2026-09-08` 的 14 天
> 挑战模型在同一测试窗口比较。正式结论、最新指标、容量和价格报告见
> `最新量化报告_20260908.md`；本文后文保留 9 月 1 日版本数字用于说明原始设计，
> 不应把旧数字当作当前生产快照。
...
本文描述的是当前已经落地（注：现行默认以代码与 采用配置（新）.md 为准；本文其余数字为 2026-09-01 设计快照，非现行生产口径）并完成离线验证的 V5 流程，不是理想化方案。文中的本轮数字来自：
...
- 非空时按接口返回的原始顺序只取第一个运营商，其余值不参与训练和容量分池。
- 首个运营商与节点原运营商一致时为本网，否则为异网。
...
当前实现不再生成“混合网络”：多值 `scheduleISPs` 依旧只按第一个运营商归类。

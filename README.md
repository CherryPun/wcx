...
**当前交付：V2 全库 nonant（时间外 n=2,994，clean 定稿）**；v1 为 5% 抽样历史口径（不作当前交付）。
数据口径：7 天累计、满 7 天账；验证采用时间外切分。
...
> V2 全库画像不走 jarvis 回填，按**宽表行属性构造**（列名须含 `bw`，jarvis 类字段以缺失处理），重建见 `07_v2_iteration/v2_data/EXEC_expansion_runbook.md`。

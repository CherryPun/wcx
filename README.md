...
> **wcx2 变更提示（重要）**：本分支（`wcx2`）已将 V5 训练默认改为**近端时效权重**
> （`V5_RECENCY_HALF_LIFE=7`、`V5_RECENCY_WEIGHT_CAP=2.0`，且 `V5_NTHREAD=1` 确定性运行），**与 RJ 原默认不同**。
> 回退到 RJ 基线：`$env:V5_RECENCY_HALF_LIFE="0"`。
> 变更依据与指标见 `采用配置（新）.md` 与 `确定性修复与重评（新）.md`；阶段汇总见 `阶段对比（新）.md`。
> 其余实验开关（`V5_TREND` / `V5_BUSINESS_BALANCE` / `V5_CALIB`）默认关闭。
...
RJ 设计快照（2026-09-01，非现行生产口径）· 历史完整训练结果：
